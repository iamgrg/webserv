...
Routes::Routes(Config const & config) : _locations(config.getLocations()), _rootPath(config.getRootPath()), _errorPages(config.getErrorPages()){
...
bool Routes::isHTTPMethod(std::string const &httpRequest){
    ...
    // Vérifier si la requête semble être cryptée (par exemple, contient des caractères non imprimables)
    for (std::string::const_iterator it = startLine.begin(); it != startLine.end(); ++it) {
        if (!isprint(*it)) {
            return false;
        }
    }
...
Response const &Routes::handle(Request const &request)
{
	...
	while(it != this->_locations.end())
	{
		if (request.getUrl() == (*it)->getPath())
		{
			...
			{
				if((*it)->getAutoindex() && !(Utils::fileExists(_rootPath + (*it)->getPath() + "index.html")))
				{
